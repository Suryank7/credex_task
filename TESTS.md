...
StackAudit's core value proposition depends on **deterministic, defensible financial math**. Every savings recommendation must be traceable to a specific pricing rule. The test suite validates the `runAudit()` function in `src/lib/audit-engine.ts` against 5 critical scenarios.
...
```
 ✓ src/__tests__/audit-engine.test.ts (5 tests)
